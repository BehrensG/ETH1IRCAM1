...
while True:
    ...
    while recevied_blocks < 4:
        ...
        data, addr = sock.recvfrom(9628)  # buffer size is 1024 bytes
    ...
    image_array = np.frombuffer(rest_of_data, dtype=np.uint16)
    image_array = (image_array.byteswap()*10)
    uint16_array = image_array.reshape((height, width))
    rescaled_image = cv2.resize(uint16_array, (width*4, height*4))
    # Display the frame
