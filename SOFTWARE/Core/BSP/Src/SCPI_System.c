...
{
	int32_t state = 0;
	int8_t password_read[STRING_LENGTH] =
	{ 0 };
	size_t length = 0;
...
scpi_result_t SCPI_SystemTemperatureUnitQ(scpi_t *context)
{
	...
	{
	case CELSIUS:
		SCPI_ResultText(context, "C");
		break;
	case FAHRENHEIT:
		SCPI_ResultText(context, "F");
		break;
	case KELVIN:
		SCPI_ResultText(context, "K");
		break;
	}
